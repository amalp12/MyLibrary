...
section .data
    message1 db 'Enter Length of Array 1 :', 10,0
    message2 db 'Enter elements of Array 1:', 10,0
    message3 db 'Enter Length of Array 2 :', 10,0
    message4 db 'Enter elements of Array 2:', 10,0
    message5 db 'The Common Elements in the Array are :', 10,0
 
    SPACE db ' '   
    NEW_LINE db 10   
    MAX_UNSIGNED_INTEGER dd 0xFFFFFFFF 

    array1 times 50 db 0
    array2 times 50 db 0
...
section .text
    global _start:
        _start:
            ...
            mov ebx, eax
            mov eax, array2
            call TakeNIntegerInputs ; now array 2 is filled with n inputs
            


            ; printing common elements in the array are
            mov eax, message5
            call printString
            ...
            call commonElementsInTwoIntegerArrays
            ...
            ; input eax - array1, ebx - length of array 1, ecx - array2, edx - length of array 2
            ; output eax - array1 replaced with common elements of array1 and array2, ebx - length of common array

            commonElementsInTwoIntegerArrays:
                sub esp , 28
                mov dword[esp + 12], edx ; length of array 2
                mov edx, esp
                mov dword[edx], eax ; array 1
                mov dword[edx + 4], ebx ; length of array 1
                mov dword[edx + 8], ecx ; array 2
                

                mov dword[edx+16], 0 ; length of common array

                mov eax, dword[edx+4] ;
                mov dword[edx+20], eax; counter
                mov eax, dword[edx] ;
                mov dword[edx+24], eax ; common array  (same as array 1)

                ; copy array one to ebx
                mov ebx, dword[edx]

                _commonElementsInTwoIntegerArraysLoop:

                    ; linear search every element of array 1 in array 2
                    movzx ecx, byte[ebx] ; element to be searched
                    
                    
                    mov eax, dword[edx + 8] ; array 2
                    push ebx ; saving array pointer for later
                    push edx
                    mov ebx, dword[edx + 12] ; length of array 2
                    call linearSearchIntegerArray

                
                    ; ebx has the boolean found or not (1 or 0)
                    cmp ebx, 1
                    pop edx
                    pop ebx ; restoring array pointer
                    je _commonElementsInTwoIntegerArraysLoopFound

                     _commonElementsInTwoIntegerArraysLoopFoundReturn:
                    ...
                    cmp eax, 0 ; comparing counter with length of array 1
  
             
                    jne _commonElementsInTwoIntegerArraysLoop

                    ; if counter is equal to length of array 1, then we have found all common elements
                    ; copy array 1 (common array now) to eax
                    mov eax, dword[edx]
                    ; move length of common array to ebx
                    mov ebx, dword[edx+16] 
                    add esp , 28
                    ret

                    
                _commonElementsInTwoIntegerArraysLoopFound:
                    
                   
                    

                    ; if found, then add it to common array
                    ; get array 1 into eax
                    mov eax, dword[edx+24]
                    movzx ecx, byte[ebx]

                    mov byte[eax], cl ; adding element to common array
                    mov eax, dword[edx+24]
                    add eax, 1
                    mov dword[edx+24],eax
                    mov eax, dword[edx+16] ; length of common array
                    add eax, 1; increment length of common array
                    mov dword[edx+16], eax ; length of common array

                    
                    jmp _commonElementsInTwoIntegerArraysLoopFoundReturn
            
           
           
           
            ; input array address in eax and the number of elements in ebx 
            ; output min element in eax
            smallestInIntegerArray:
    
                mov ecx, ebx ; setting counter
                mov ebx, eax ; moving array to ebx for convinience 
                movzx edx, byte[ebx] ; getting the first element of the array
                ; edx hold the current largest element
                _smallestInIntegerArrayLoop:
                    ...
                    jnz _smallestInIntegerArrayLoop
            ...
            ; input array address in eax and the number of elements in ebx 
            ; output max element in eax
            largestInIntegerArray:
    
                mov ecx, ebx ; setting counter
                mov ebx, eax ; moving array to ebx for convinience 
                movzx edx, byte[ebx] ; getting the first element of the array
                ; edx hold the current largest element
                _largestInIntegerArrayLoop:
                    ...
                    sub ecx,1 ; decrementing the counter
                    cmp ecx, 0 ; checking if the counter is zero
                    jnz _largestInIntegerArrayLoop
